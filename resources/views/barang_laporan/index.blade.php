    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                ...
                <x-primary-button tag="a" href="{{ route('barang_laporan.print') }}">Cetak PDF</x-primary-button>
                    <!-- Tabs -->
                    ...
                    <div id="belum-diterima" class="tab-content {{ $activeTab == 'belum-diterima' ? '' : 'hidden' }}">
                        <x-table>
                            ...
                            <tr>
                                ...
                                <td>{{$b->jml_barang_terlapor}}</td>
                                <td>{{$b->keterangan}}</td>
                                <td>{{$b->status}}</td>
                                <td>{{$b->user->name}}</td>
                                <td>{{$b->created_at->format('d-m-y')}}</td>
                                <td>
                                    <x-primary-button tag="a" href="{{ route('laporan_barang.terima', $b->id) }}">Terima Laporan</x-primary-button>
                                </td>
                            </tr>
                            @endforeach
                        </x-table>
